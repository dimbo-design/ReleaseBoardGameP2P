...
const ISSUES: Issue[] = [
  ...
  },
  {
    ...
    problem: {
      ru: '`packages/engine/src/events.ts` обещал на `EventBase.parent`: «a defence names the attack it answered … so the history tree needs no inference» — это было намерение, а не поведение: ни один `defended` не нёс `parent`, `attackerOf` (`toBoardState.ts`) в проде всегда уходил в раннюю ветку, и `returnCard`/`redirect` вместе с вложенностью атака/защита были недостижимы на живом столе, пока четыре теста с вручную написанным `parent: 1` показывали обратное (#136). ИСПРАВЛЕНО В КОДЕ (#139), НЕ ЗАКРЫТО: id `attacked` едет на пендинге защиты, и его называют и `defended`, и `tookHit`; у DDoS — его никто не отвечает, а цель выбирает разыгрывающий — свой `attacked` называют `monitoringDestroyed`/`releaseReturned` и потраченная карта. `attackDefenceHistory.test.ts` гоняет настоящий движок через адаптер. Закрыть находку — решение дизайнера (ревью #139), поэтому статус `open`. См. `docs/animations/backlog.md`.',
      en: '`packages/engine/src/events.ts` promised on `EventBase.parent`: "a defence names the attack it answered … so the history tree needs no inference" — an intent, not a behaviour: no `defended` carried a parent, so `attackerOf` (`toBoardState.ts`) always took its early branch in production, and `returnCard`/`redirect` and the attack/defence nesting were unreachable on a live table while four tests with a hand-written `parent: 1` reported them working (#136). FIXED IN CODE (#139), NOT CLOSED: the `attacked` id rides the defend pending and both `defended` and `tookHit` name it; for a DDoS, which nothing answers and whose target the thrower chooses, `monitoringDestroyed`/`releaseReturned` and the spent card name their `attacked`. `attackDefenceHistory.test.ts` drives the real engine through the adapter. Closing the finding is a decision for the designer (review on #139), so the status stays `open`. See `docs/animations/backlog.md`.',
    },
    where: {
